...
void CAN_init(void){
	...
	mcp2515_bit_modify(MODE_MASK, MODE_LOOPBACK, MCP_CANCTRL);
	CAN_interrupt_init();
}
...
void CAN_send_message(can_message msg) {
	...
	// write id
	uint8_t id_high = (uint8_t) ((msg.id & 0b11111111000) >> 3);
	uint8_t id_low = (uint8_t) ((msg.id & 0b111) << 5);
	
	mcp2515_bit_modify(0xE0, id_low, MCP_TXB0SIDL);
	mcp2515_write(id_high, MCP_TXB0SIDH);
	
	// write length
	mcp2515_bit_modify(0x0F, msg.length, MCP_TXB0DLC);
	
	// write data
	for(int byte = 0; byte < msg.length; byte++){
		mcp2515_write(msg.data[byte], MCP_TXB0D0+byte);
	}
