...
int mov_read_button(button b){
	...
	switch (b){
		case jb:{
			if(DEBUG_BUTTON)printf("Reading joystick button:\r\n");
			button_pushed = !(PINB & (1 << PB2));
			break;
		}
		case rb:{
			if(DEBUG_BUTTON)printf("Reading right slider button:\r\n");
			button_pushed = PINB & (1 << PB0);
		}
		case lb:{
			if(DEBUG_BUTTON)printf("Reading left slider button:\r\n");
	...
	if (button_pushed){
		if(DEBUG_BUTTON)printf("Button pressed\r\n");
		return 1; 
	}
	return 0; 
}
...
input_j mov_get_joy_input(void){
	...
	joy_input.button_pressed = mov_read_button(jb);
...
void mov_send_can_message(int CAN_ID){
	...
	switch (CAN_ID){
		case CAN_JOYSTICK_ID:{
			...
			movement_msg.data[2] = joystick_input.button_pressed;
